...
export function SocialLinksSection({ 
  ...
  const togglePlatform = (platform: string) => {
    const newEnabledPlatforms = new Set(enabledPlatforms)
    if (newEnabledPlatforms.has(platform)) {
      newEnabledPlatforms.delete(platform)
      // Remove the platform from social_links if it exists
      const newLinks = social_links.filter(link => 
        !link.toLowerCase().includes(platform.toLowerCase())
      )
      onUpdate('social_links', newLinks)
    } else {
      newEnabledPlatforms.add(platform)
      // Add the platform to social_links if it doesn't exist
      if (!social_links.some(link => link.toLowerCase().includes(platform.toLowerCase()))) {
        const newLinks = [...social_links, platform]
        onUpdate('social_links', newLinks)
      }
    }
  ...
  const updateSocialLink = (platform: string, value: string) => {
    ...
    const existingIndex = newLinks.findIndex(link => 
      link.toLowerCase().includes(platform.toLowerCase())
    )
          ...
          <div className="space-y-2">
            {platforms.map((platform) => {
              const isEnabled = enabledPlatforms.has(platform) || 
                social_links.some(link => link.toLowerCase().includes(platform.toLowerCase()))
              
              // Initialize enabled platforms from existing social links
              if (isEnabled && !enabledPlatforms.has(platform)) {
                setEnabledPlatforms(prev => new Set([...prev, platform]))
              }
